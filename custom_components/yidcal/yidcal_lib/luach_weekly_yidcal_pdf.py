...
# Design fonts (supplied by Yoel): BAFranknatan for all Hebrew text,
# Bahnschrift for the grid/civil/small-box digits (Acumin stand-in),
# Miriam for the \u00a9YidCal mark. BAFranknatan is a subset face —
# it lacks geresh/gershayim/hyphen/digits, so Miriam then Heebo are
...
_GRID_FILE = "Bahnschrift-SemiBold.ttf"   # semibold only — every
                                          # grid/date/time draw and
                                          # the watermark use it
_MARK_FILE = "Miriam.ttf"
...
def register_design_fonts(pdf: FPDF) -> None:
    ...
    pdf.set_fallback_fonts([FONT_MARK, FONT_FAMILY],
                           exact_match=False)
...
class _Weekly2PDF(FPDF):
    ...
    def _mark_w(self) -> float:
        ...
        self.set_font(FONT_MARK, "", self._S(_MARK_SIZE))
