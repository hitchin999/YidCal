...
design mock PDFs, which carry a live text layer). Hebrew text uses the
bundled YidCalHebrew face,
Latin, digits, and the \u00a9YidCal mark use Archivo.
...
_MARK_BASE_Y = 753.3             # mock baseline of the \u00a9YidCal mark
...
# ── folded ל ─────────────────────────────────────────────────────────
# YidCalHebrew maps a folded ל at U+E004 (its ascender curled into a
# teardrop, topping at 0.656 em vs the plain ל's 0.828). 
...
_FOLDED_LAMED = "\ue004"       # folded ל in YidCalHebrew


@lru_cache(maxsize=None)
def _heb_has_folded_lamed() -> bool:
    """True iff the shipped Hebrew face maps U+E004 (the folded ל).

    YidCalHebrew carries it; a replacement face may not. When it does
    not, _fold_lameds becomes a no-op and candle labels use the plain ל
    — no missing-glyph boxes."""
    try:
        from fontTools.ttLib import TTFont
        f = TTFont(str(_fonts_dir() / _HEB_FILE))
        return ord(_FOLDED_LAMED) in f.getBestCmap()
    except Exception:
        return False
...
_LAM_ASC_PLAIN, _LAM_ASC_FOLD = 0.828, 0.656   # YidCalHebrew ל / folded ל
_FOLD_LIFT_EM = (_LAM_ASC_PLAIN - _LAM_ASC_FOLD) / 2.0      # 0.0855


# ── ASCII quotes inside Hebrew words ──────────────────────────────────
# The Hebrew face (YidCalHebrew) carries its own ״ ׳ at the right height,
# so marks render in it directly. But callers still hand us raw ASCII in
# places (the service builds its strip as 'לפ"ק'), and an ASCII " would
# render as a Latin quote via the fallback — so normalise ASCII quotes
# INSIDE Hebrew words to the real marks. _t is the single choke point.
...
def _fold_lameds(text: str) -> str:
    """Fold EVERY ל in a candle-lighting label — if the font has the glyph.

    Gated on the label actually being a candle box ('הדלקת' / 'הדלה״נ')
    — the check runs BEFORE the substitution, since the token itself
    contains a ל. No-op when the loaded Hebrew face lacks the folded ל,
    so a replacement font degrades to its plain ל cleanly."""
    if not text or not any(tok in text for tok in _CANDLE_TOKENS):
        return text
    if not _heb_has_folded_lamed():
        return text
...
# Design fonts: YidCalHebrew for all Hebrew text (letters, ״ ׳, and the
# folded ל at U+E004), Archivo for the grid/civil/small-box digits +
# watermark + \u00a9YidCal mark. YidCalHebrew has no Latin, so Archivo
# then Heebo are registered as glyph fallbacks; Heebo is also the
# ultimate fallback for any glyph (e.g. nikud) the design face lacks.
#
...
_HEB_FILE = "YidCalHebrew-Regular.otf"
_GRID_FILE = "Archivo-SemiCondensed-SemiBold.ttf"  # every
                                          # grid/date/time draw and
                                          # the watermark use it
_MARK_FILE = "Heebo-Regular.ttf"   # nikud + last-resort glyph fallback
...
# YidCalHebrew's Hebrew letters (matched at 1.00x) — so
# the English place name and the molad's digits looked
...
def register_design_fonts(pdf: FPDF) -> None:
    ...
    # re-checks YidCalHebrew before anything else, so Hebrew can never be
    ...
    #   FONT_MARK (Heebo)    — nikud + last-resort glyph safety net
    #                          (the ״ ׳ now come from FONT_HEB itself)
...
class _Weekly2PDF(FPDF):
    ...
    def _t(self, cx, mid_y, s, *, size, color=(0, 0, 0), anchor="C",
        ...
        if baseline is not None:
            ...
            # the YidCalHebrew face (which lacks digits, -, •).
    ...
    def _yidcal_mark(self, right_x: float) -> None:
        """Stamp the \u00a9YidCal brand mark (Archivo, at the Hebrew
        footnote size; mock baseline 753.3) ending at ``right_x``."""
        self.set_font(FONT_LAT, "B", self._S(_MARK_SIZE))
    ...
    def _footer(self) -> None:
        ...
        # The \u00a9YidCal brand mark is stamped separately (in Archivo,
