...
Layout: A4 portrait, RTL Hebrew table. Each LuachRow is a single row
...
from .luach_pdf_common import register_fonts, bidi, FONT_FAMILY
...
class _YearlyMultiPagePDF(FPDF):
    ...
    def _draw_watermark(self) -> None:
        """Draw a sideways watermark along the left edge of every page.

        Uses ``local_context()`` so font/color changes don't leak into
        the rest of header() / row rendering, and the page cursor is
        unaffected because we use ``text()`` (which doesn't advance
        the cursor) rather than ``cell()``.

        Layout: text is rotated 90° counter-clockwise around its
        starting point, so it reads bottom-to-top along the left
        margin. The pivot ``cy`` is chosen so the rotated text is
        ...
        with self.local_context():
            ...
            cx = 5.0                       # 5 mm in — clears the crop when
                                           # A4 output prints on Letter stock
            cy = (self.h + text_w) / 2     # bottom of the rotated text
