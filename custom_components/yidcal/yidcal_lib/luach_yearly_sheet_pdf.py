...
Layout: US Letter portrait, RTL Hebrew, two columns. A single page covers an
...
from .luach_data import (
    LuachRow, AnnotationRow, LuachItem, PRUZBOL_FOOTNOTE_HE,
)
...
class _YearlySheetPDF(FPDF):
    ...
    PAGE_HEIGHT_MM = 279.4        # US Letter height (11 in)
    LEFT_MARGIN = 8
    RIGHT_MARGIN = 8
    TOP_MARGIN = 8
    BOTTOM_MARGIN = 10
    ...
    ROW_HEIGHT = 4.0
    ANN_HEIGHT = 3.5
    PRUZBOL_GAP = 1.2       # breathing room above the נוסח footnote
    ...
    # ── Watermark (rotated, left edge) ─────────────────────────────
    def _draw_watermark(self) -> None:
        """Draw the shared "Generated by YidCal" watermark up the left
        edge (Archivo SemiBold, light grey, rotated, vertically centred
        — identical to the Weekly YidCal card and the yearly-multi-page
        style). See ``luach_pdf_common.draw_watermark``.
        """
        draw_watermark(self)
    ...
    def _draw_title_cell(
        ...
        def _is_loose_special(lbl: str) -> bool:
            # A parenthetical (the Friday's own YT identity on a
            # Friday-is-a-YT-day row) prints AFTER the Hebrew date:
            #   'האזינו שובה, ב׳ תשרי, (ב׳ דראש השנה)'
            if lbl.startswith("("):
                return True
    ...
    def _annotation_lines(self, ann: AnnotationRow) -> list[str]:
        ...
        are NOT split).
        """
        # A sheet-specific wording wins when the data layer supplies one
        # (the SF sheet abbreviates + breaks differently from the weekly
        # card). Explicit '\n' = a hard line break.
        sheet_text = getattr(ann, "text_sheet_he", "") or ""
        if sheet_text:
            return [ln.strip() for ln in sheet_text.split("\n")
                    if ln.strip()]
        text = ann.text_he or ""
    ...
    def _draw_annotation(
        ...
        base_size = self.DST_SIZE if is_dst else self.ANN_SIZE
        max_w = col_w - 0.6
        for line in self._annotation_lines(ann):
            line_bidi = bidi(line)
            # Keep the line inside its own column — a long one used to
            # spill across the gutter and collide with the other
            # column's zmanim (the Erev-Pesach-on-Shabbos chametz line).
            self.set_stretching(100)
            self.set_font(
                FONT_FAMILY_SERIF, "B" if is_dst else "", base_size)
            size = base_size
            w = self.get_string_width(line_bidi)
            if w > max_w:
                self.set_stretching(max(62.0, 100.0 * max_w / w))
                w = self.get_string_width(line_bidi)
                _g = 0
                while w > max_w and size > 5.0 and _g < 40:
                    size -= 0.25
                    self.set_font(
                        FONT_FAMILY_SERIF,
                        "B" if is_dst else "", size)
                    w = self.get_string_width(line_bidi)
                    _g += 1
    ...
    def _item_height(self, item: LuachItem) -> float:
        if isinstance(item, LuachRow):
            if item.candle_kind == "motzaei_sh_to_yt":
                # Shabbos-Erev-Pesach: the printed sheet has no row for
                # it (the Friday row carries '(שבת ער״פ)'), so it takes
                # no space. The WEEKLY still consumes the row.
                return 0.0
    ...
    def _compute_column_break(self, items: list[LuachItem]) -> int:
        ...
        for i in range(1, len(items)):
            ...
            if (isinstance(cur, LuachRow)
                and cur.candle_kind != "motzaei_sh_to_yt"):
    ...
    def render_body(self, items: Iterable[LuachItem]) -> None:
        ...
        # Body geometry.
        #
        # Each column draws a FIXED block of cells whose width is the sum
        # of the per-field widths (W_TITLE … W_ALOS); the two columns plus
        # one COL_GAP make a combined block whose width does NOT depend on
        # the page size. We therefore centre that fixed block within the
        # usable width and let any surplus page width (US Letter is wider
        # than the old A4 target) fall into equal outer margins. Deriving
        # the column width from the page instead would pump the surplus
        # into the inter-column gap and push the whole block off-centre
        # (it looks shifted toward the right rule).
        usable_w = self.PAGE_WIDTH_MM - self.LEFT_MARGIN - self.RIGHT_MARGIN
        self._col_width = (
            self.W_TITLE + self.W_CIVIL + self.W_CANDLE
            + self.W_MOTZEI + self.W_SOFZMAN + self.W_ALOS
        )
        block_w = 2 * self._col_width + self.COL_GAP
        outer = (usable_w - block_w) / 2.0   # equal margin each side of block
        # Right column right edge = block centred in the usable width, then
        # pulled left by COLUMN_SHIFT_LEFT to optically centre the ink
        # between the rules (titles end flush right; the alos digits carry
        # intrinsic left air — see the constant's comment).
        right_col_right = (
            self.PAGE_WIDTH_MM - self.RIGHT_MARGIN - outer
            - self.COLUMN_SHIFT_LEFT
        )
        ...
        # The pruzbol footnote prints ABOVE the closing double rule
        # (as on the printed sheet), so reserve its band up-front —
        # the columns shorten by that much instead of running into it.
        self._pruzbol_h = (
            self.ANN_HEIGHT + 2 * self.PRUZBOL_GAP
            if any(isinstance(it, AnnotationRow) and it.kind == "pruzbol"
                   for it in items_list)
            else 0.0
        )
        self._body_height -= self._pruzbol_h
        ...
        body_end_y = self._draw_pruzbol_footnote(items_list, body_end_y)
        self._draw_double_rule(body_end_y)

    def _draw_pruzbol_footnote(
        self, items: list[LuachItem], y: float,
    ) -> float:
        """The נוסח-הפרוזבול footnote, printed at the foot of the
        sheet ONLY in a year whose Erev RH closes a shmita year —
        i.e. exactly when the 'צריכין לעשות פרוזבול*' note appears
        in the body (SF 5783). Both are driven by the one predicate
        halacha_events.needs_pruzbol().""" 
        if not any(isinstance(it, AnnotationRow) and it.kind == "pruzbol"
                   for it in items):
            return y
        y += self.PRUZBOL_GAP
        text = bidi(PRUZBOL_FOOTNOTE_HE)
        self.set_text_color(40, 40, 40)
        self.set_font(FONT_FAMILY_SERIF, "", self.ANN_SIZE)
        avail = (self.PAGE_WIDTH_MM - self.LEFT_MARGIN
                 - self.RIGHT_MARGIN)
        # condense before shrinking — the נוסח is one long line
        size = self.ANN_SIZE
        while size > 5.0 and self.get_string_width(text) > avail:
            size -= 0.25
            self.set_font(FONT_FAMILY_SERIF, "", size)
        w = self.get_string_width(text)
        self.set_xy(self.LEFT_MARGIN + (avail - w) / 2.0, y)
        self.cell(w, self.ANN_HEIGHT, text, align="C")
        return y + self.ANN_HEIGHT + self.PRUZBOL_GAP
    ...
    def _draw_column(
        ...
        *,
        ...
        for item in items:
            if isinstance(item, LuachRow):
                if item.candle_kind == "motzaei_sh_to_yt":
                    # Shabbos-Erev-Pesach — the printed sheet prints no
                    # row for it (the Friday row carries '(שבת ער״פ)').
                    # The WEEKLY still consumes this row, so it stays in
                    # the stream; we simply don't draw it.
                    continue
